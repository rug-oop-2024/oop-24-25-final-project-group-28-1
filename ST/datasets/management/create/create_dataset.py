...
stored_dataset = st.selectbox("or select an existing file:", 
                               csvfiles, 
                               index=None,
                               placeholder="Select a file from this drop down menu",
                               help=helpmes, key="selectbox")
if stored_dataset == "None":
    stored_dataset = None
    
st.write("### 3. File uploader (from local file system):")
uploaded_file = st.file_uploader("or upload a file", type="csv")  # Sets the file manager's filter to ".csv"
if uploaded_file:
    url = ""
    stored_dataset = ""
...
if not df is None:        
    ...
    if dataset_name:        
        ...
        dataset = Dataset.from_dataframe(df, name=dataset_name, asset_path=dataset_name, id=newid) 
